...
class Kernels:
    ...
    '''
    def __init__(self,vza,sza,raa,critical=1,RossHS=True,RecipFlag=True,HB=2.0,BR=1.0,MODISSPARSE=True,MODISDENSE=False,RossType='Thick',normalise=1,normalize=0,LiType='Transit',doIntegrals=True,BSAangles=[],nbar=45.0):
        '''
...
def testLisa(inputFile,buff=30,LiType='Sparse',RossType='Thick',plot=False,verbose=False):
...
def lisaInvert(vza,sza,raa,refl,doy,dos,LiType='Sparse',RossType='Thick',xi=False):
    doy2 = doy*doy
    doy3 = doy2*doy
...
# test function
def mimic(doPrint=False,doPlot=False,RossHS=False):
    '''
	...
	RAA[i,:] = 0.0
        # do the kernels
        kk = Kernels(VZA[i,:] ,SZA[i,:],RAA[i,:],RossHS=RossHS,MODISSPARSE=True,RecipFlag=False,normalise=1,doIntegrals=False,LiType='Dense',RossType='Thick')
    	RossThick[i,:] = kk.Ross[:]
        LiDense[i,:] = kk.Li[:]
 	if doPrint == True:
		kk.printKernels(file='RossThickLiDense.' + str(SZAS[i]) + '.dat')
		kk.printer('')
	kk = Kernels(VZA[i,:] ,SZA[i,:],RAA[i,:],RossHS=RossHS,MODISSPARSE=True,RecipFlag=False,normalise=1,doIntegrals=False,LiType='Sparse',RossType='Thin')
	RossThin[i,:] = kk.Ross[:]
        LiSparse[i,:] = kk.Li[:]
        if doPrint == True:
                kk.printKernels(file='RossThinLiSparse.' + str(SZAS[i]) + '.dat')
                kk.printer('')
	kk = Kernels(VZA[i,:] ,SZA[i,:],RAA[i,:],RossHS=RossHS,MODISSPARSE=True,RecipFlag=False,normalise=1,doIntegrals=False,LiType='Roujean',RossType='Thin')
	Roujean[i,:] = kk.Li[:]
	if doPrint == True:
                kk.printKernels(file='RossThinRoujean.' + str(SZAS[i]) + '.dat')
                kk.printer('')
        kk = Kernels(VZA[i,:] ,SZA[i,:],RAA[i,:],RossHS=RossHS,MODISSPARSE=True,RecipFlag=False,normalise=1,doIntegrals=False,LiType='Transit',RossType='Thin')
        LiTransit[i,:] = kk.Li[:]
        if doPrint == True:
	...
	#for i in range(len(SZAS)):
	    ...
	    #pylab.plot(x,y)
	    #pylab.plot(kk.vzaDegrees,RossThick[i,:])
            #pylab.plot(kk.vzaDegrees,RossThin[i,:])
            #pylab.plot(kk.vzaDegrees,LiSparse[i,:])
            #pylab.plot(kk.vzaDegrees,LiDense[i,:])
            #pylab.plot(kk.vzaDegrees,Roujean[i,:])
            #pylab.plot(kk.vzaDegrees,LiTransit[i,:])
...
if __name__=="__main__":
	inputFile = 'data.r2023.c87.dat'
	doy,minrmse,minP,minFWD,minrefl,mindoy,sig,rel,fcc,a0,a1 =\
			testLisa(inputFile)#,verbose=True,plot=True)
